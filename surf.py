...
img1 = plt.imread("img_4/img1.jpg")
img2 = plt.imread("img_4/img2.jpg")
...
# plt.imsave("surf待拼接的两张图.png", daipinjie_image)

start_time = time.time()
# 使用SURF检测关键点，并在原图像上绘制关键点，并打印特征点数目
...
# plt.imsave("surf特征提取.png", tezhengtiqu_image)
...
img_surf = cv.drawMatchesKnn(img1, key1, img2, key2, [good_matches], None, flags=2) 
# plt.imshow(img_surf),plt.axis('off'), plt.show()
...
if len(good_matches) > MIN:
    ...
    # 保存拼接后的图像
    plt.imsave("surf原图.png", daipinjie_image)
    plt.imsave("surf特征提取.png", tezhengtiqu_image)
    plt.imsave("surf特征匹配.png", img_surf)
    plt.imsave("surf直接拼接.png", direct)
    plt.imsave("surf对边缘进行平滑处理.png", warpImg)
